...
func New(q *postgres.Queries, weeksAhead int) *Generator {
	return &Generator{
		queries:    q,
...
func (g *Generator) GenerateEvents(ctx context.Context, log *slog.Logger) error {
...
func (g *Generator) processStudent(ctx context.Context, student postgres.Student, log *slog.Logger) error {
...
func (g *Generator) calculateInitDate(orderDay int16) (time.Time, error) {
	now := time.Now()
...
func (g *Generator) eventExists(ctx context.Context, studentID int64, date time.Time, log *slog.Logger) (bool, error) {
...
func (g *Generator) createEvent(ctx context.Context, student postgres.Student, date time.Time, log *slog.Logger) error {
	var pgDate pgtype.Date
	if err := pgDate.Scan(date); err != nil {
		log.Debug("date conversion error: ", sl.Err(err))
		return err
